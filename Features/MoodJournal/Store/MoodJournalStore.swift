...
final class MoodJournalStore {
    ...
    var weeklyOverview: WeeklyMoodOverview {
        Self.weeklyOverview(for: entries, referenceDate: Date())
    }

    var allSelectedMoodOptions: [String] {
        Array(Set(entries.flatMap(\.selectedEmotions).map { $0.capitalized }))
            .sorted()
    }

    var weeklyEntries: [JournalEntry] {
        entriesThisWeek(referenceDate: Date()).sorted { $0.createdAt > $1.createdAt }
    }

    var weeklyMoodTrends: [MoodTrendDatum] {
        Self.weeklyMoodTrends(for: entries, referenceDate: Date())
    }
